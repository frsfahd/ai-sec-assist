...
async def message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text=str(response.content)
    )
...
async def media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mediaList[update.message.from_user.username] = {
        update.message.id: {"file_id": doc.file_id, "filename": doc.file_name}
    }
...
async def messageWithMedia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    thread = mediaList[update.message.from_user.username][
...
if __name__ == "__main__":
    ...
    messageWithMediaHandler = MessageHandler(
        filters.TEXT & filters.REPLY & (~filters.COMMAND), messageWithMedia
    )
